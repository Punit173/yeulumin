  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-[#0A0A0A] text-[#f5f5f5]">
      ...
      <main className="flex-grow mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Filter bar */}
        <div className="flex flex-col gap-6 bg-[#111111]/30 border border-neutral-900 rounded-xl p-6 backdrop-blur-md mb-8">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            ...
            <div className="relative flex-grow max-w-md">
              <input
                ...
                className="w-full bg-[#111111] border border-neutral-800 focus:border-neon focus:ring-1 focus:ring-neon rounded-lg py-2.5 pl-10 pr-4 text-xs text-neutral-300 placeholder-neutral-600 focus:outline-none transition-all"
              />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-[#111111] border border-neutral-800 hover:border-neutral-700 text-neutral-300 text-xs rounded-lg pl-3 pr-8 py-2.5 focus:outline-none focus:border-neon transition-colors cursor-pointer"
                >
              ...
              return (
                <button
                  ...
                  className={`text-xs font-semibold px-4 py-2 rounded-lg border transition-all duration-200 uppercase tracking-wider whitespace-nowrap cursor-pointer ${
                    isActive
                      ? "bg-neon border-neon text-[#0A0A0A] font-bold glow-neon"
                      : "bg-[#111111]/40 border-neutral-900 text-neutral-400 hover:border-neutral-700 hover:text-white"
                  }`}
                >
                  {cat}
                </button>
        ...
        {processedProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {processedProducts.map((product) => (
              <div
                key={product.id}
                className="bg-[#111111]/30 border border-neutral-900 hover:border-neon/30 rounded-xl overflow-hidden group flex flex-col justify-between transition-all duration-300"
              >
                <div>
                  {/* Image wrapper */}
                  <div className="relative aspect-square w-full bg-neutral-950 overflow-hidden">
                    {/* Badge */}
                    <span className="absolute top-3.5 left-3.5 z-10 text-[9px] font-mono font-bold tracking-widest uppercase bg-[#0A0A0A]/90 border border-neutral-800 text-neon px-2 py-0.5 rounded">
                      {product.badge}
                    </span>
                  ...
                  {/* Detail contents */}
                  <div className="p-6">
                    <span className="text-[10px] uppercase font-mono tracking-widest text-neutral-500 block mb-1.5">
                      {product.category}
                    </span>
                    <div className="flex justify-between items-baseline mb-3">
                      <h2 className="font-bold text-neutral-200 text-base group-hover:text-white transition-colors">
                        {product.name}
                      </h2>
                      <span className="font-mono text-neon font-bold text-base whitespace-nowrap ml-2">
                        ₹{product.price}
                      </span>
                    </div>
                ...
                <div className="px-6 pb-6 pt-2 border-t border-neutral-900/30 flex flex-wrap gap-1.5">
                  {product.tags.map((tag) => (
                    <span
                      key={tag}
                      onClick={() => setSearchQuery(tag)}
                      className="text-[9px] font-mono text-neutral-500 bg-neutral-950 px-2 py-1 rounded border border-neutral-900/80 hover:text-neon hover:border-neon/30 transition-colors cursor-pointer"
                    >
                      #{tag}
                    </span>
                  ))}
          ...
          <div className="border border-dashed border-neutral-900 rounded-xl py-24 text-center max-w-md mx-auto mt-12 flex flex-col items-center gap-4">
            <SlidersHorizontal className="h-10 w-10 text-neutral-700 animate-pulse" />
